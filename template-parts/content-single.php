...
<?php } ?>
	...
	<div class="entry-content">
		...
		$link = get_field('product_link');	
			...
			if (!empty($image)) {				
				echo '<a href="' . $link . '" rel="nofollow" target="_blank"><img src="' . $image['sizes']['large'] . '" alt="' . get_the_title() . '" class="product-image"></a>';				
			}
		//echo $image['url'];
		if (!empty($link)) {		
		?>
		...
		<div class="row">			
			<div class="col product-link">
				...
				<?php the_field('product_price'); ?> - Check it out &raquo;
				</a>	
			</div>
		</div><!--/.row-->	
		<?php
			...
			the_content( sprintf(
				...
				wp_kses( __( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'skyhigh' ), array( 'span' => array( 'class' => array() ) ) ),
